...
NODES = [("localhost", 8000), ("localhost", 8001), ("localhost", 8002)] # In the cloud, update "localhost" to each nodes' corresponding internal IP
...
# Define a Threaded XML-RPC server to handle requests in parallel
...
class Node:
    def __init__(self, node_id, port):
        ...
        self.rpc_server = ThreadedXMLRPCServer(("localhost", port), allow_none=True) # In the cloud, "localhost" should be the internal IP of the node its running on
    ...
    def prepare_phase(self, value):
        ...
        for attempt in range(retries):
            ...
            if promise_count >= MAJORITY:
                ...
                self.proposed_id += random.randint(1, 5)
                print(f"[Node {self.node_id}] Backing off for {backoff_time:.2f} seconds before retrying prepare with new ID: {self.proposed_id}.")
    ...
    def update_cisc5597(self, value):
        ...
        self.consensus_reached = True # Consensus has been reached
...
if __name__ == "__main__":
    ...
    # Parse command-line arguments for node_id and port
    parser = argparse.ArgumentParser(description="Start a Paxos Node.")
    parser.add_argument("--node_id", type=int, required=True, help="The unique ID of the node.")
    parser.add_argument("--port", type=int, required=True, help="The port for the node.")
    args = parser.parse_args()

    # Start the node
    node = Node(args.node_id, args.port)
...
# Ensure that each node_id corresponds to its given port
